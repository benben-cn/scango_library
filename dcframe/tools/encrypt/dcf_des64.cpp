...
void DES_CBCInit(DESX_CBC_CTX *context, unsigned char *key, unsigned char *iv, int encrypt);
int DES_CBCUpdate(DESX_CBC_CTX *context, unsigned char *output, unsigned char *input, unsigned int len);
void DES_CBCRestart(DESX_CBC_CTX *context);
...
#else
/* CustomTabSimpleBean box tables for assembler desfunc */
...
int DES3_CBCUpdate(DES3_CBC_CTX *context, unsigned char *output, unsigned char *input, unsigned int len){
    ...
    for(i = 0; i < len; i += 8) {
        if (( i + 8) > len)
        {
            ...
            memcpy(inputtmp, &input[i], len - i);
            scrunch(inputBlock, inputtmp);
        }
        else
        {
            scrunch(inputBlock, &input[i]);
        }
...
static const BYTE zhw_des3_iv[8] = { 'z','H','w','I','V','k','e','Y' };
static const BYTE zhw_default_pass[24] = { 'z','H','w','D','P','a','s','S',0x10,0x20,0x30,0x40,0x50,0x60,0x70,0x80,0x90,0xa0,0xb0,0xc0,0xd0,0xe0,0xf0,0x15};
static void des_copy_pass(BYTE *keybuf, const char *key)
{
    int i = 0;
    const BYTE *src = (BYTE*)key;
    for (; i < 24; i++)
    {
        if (!src[i])
        {
            src = zhw_default_pass;
        }
        keybuf[i] = src[i];
    }
}
DWORD dcf_tools_3des64_encrypt(BYTE* input, DWORD datalen, BYTE* output, const char* key)
{
    BYTE keytmp[24];
    des_copy_pass(keytmp, key);
    DES3_CBC_CTX context;
    DES3_CBCInit(&context, (BYTE*)keytmp, (BYTE*)zhw_des3_iv, 1);
    return DES3_CBCUpdate(&context, output, input, datalen);
}
DWORD dcf_tools_3des64_decrypt(BYTE* input, DWORD length, BYTE* output, const char* key)
{
    BYTE keytmp[24];
    des_copy_pass(keytmp, key);
    DES3_CBC_CTX context;
    DES3_CBCInit(&context, (BYTE*)keytmp, (BYTE*)zhw_des3_iv, 0);
    return DES3_CBCUpdate(&context, output, input, length);
}