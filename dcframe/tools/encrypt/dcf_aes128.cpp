...
WORD dcf_tools_crc16_get(BYTE *q,int ilen)
{
    unsigned short crc = 0;
    while (ilen-- > 0)
        crc = dcf_ccitt_table[(crc >> 8 ^ *q++) & 0xff] ^ (crc << 8);
    return ~crc;
}
...
// Substitutes a word using the AES CustomTabSimpleBean-Box.
...
static const BYTE g_ces128iv[]        = { 0xa0, 0xb1, 0xc2, 0xb3, 0xd4, 0xe5, 0xf6, 0x7a, 0x8b, 0x9c, 0xad, 0xbe, 0xef, 0xff, 0x00, 0xff };
static const BYTE g_ces128ivkey[]  = { 0xb5, 0xd3, 0xb5, 0xca, 0xd3, 0xd6, 0xb9, 0xa1, 0xf6, 0xe3, 0xba, 0xf4, 0xda, 0xd0, 0xfa, 0xa3};
static void get_my_key(const char *key,BYTE byBuffer[16])
{
    for(int i = 0;i<16;i++)
    {
        if (*key)
...
{
    BYTE byMyKey[16];
    get_my_key(key,byMyKey);
    UINT cvtKey[44] = {0};
    aes_key_setup(byMyKey,cvtKey,128);
    return aes_encrypt_cbc(input,datalen,output,cvtKey,128,g_ces128iv);
}
DWORD dcf_tools_ces128_decrypt(BYTE* input, DWORD length,BYTE* output,const char* key)
{
    ...
    UINT cvtKey[44] = {0};
    aes_key_setup(byMyKey,cvtKey,128);
    return aes_decrypt_cbc(input,length, output,cvtKey,128,g_ces128iv);
}
